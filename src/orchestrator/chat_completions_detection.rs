...
use futures::{future::{BoxFuture, try_join_all}, Future};
...
// pub type ChunkResult = Pin<Box<dyn Future<Output = ChatMessagesInternal> + Send>>;
pub type ChunkResult = BoxFuture<'static, ChatMessagesInternal>;
        ...
        let task_handle = tokio::spawn(async move {
            ...
            let input_detections = match input_detectors {
                Some(detectors) if !detectors.is_empty() => {
                    ...
                    input_detection(&ctx, &detectors, chunks, headers.clone()).await.unwrap()
                }
                _ => None,
            };
        });
...
pub async fn input_detection(
    ...
    chunks: Arc<HashMap<String, ChunkResult>>,
    headers: HeaderMap,
    ...
    let tasks = detectors
        ...
        .map(|(detector_id, detector_params)| {
            ...
            // let messages = chunks.get(chunker_id).ok_or_else(|| {
            //     Error::Other(format!(
            //         "Chunk for {chunker_id} not found"
            //     ))
            // })?;
            let headers = headers.clone();

            let chunk = Arc::clone(&chunks);
            async move {
                // let messages = chunk.get(chunker_id)?.await;
                // if let Some(messages) = chunk.get(&detector_id) {
                //     let chunk = messages.into_future().await;
                // }

                if let Some(messages) = chunk.get(chunker_id) {
                    // let chunk_messages = messages.deref().into().await;
                    let chunk_messages = messages.as_ref().await;
                    match detector_type {
                    ...
                    Err(Error::Other(format!("Chunk for {chunker_id} not found")))
                }
...
fn filter_chat_messages(
    ...
    .map(|(detector_id, _)| {
        ...
        match detector_type {
            DetectorType::TextContents => {
                match content::filter_chat_message(messages) {
...
fn detector_chunk_task(
    ...
    chunker_ids: Vec<String>) -> Result<HashMap<String, ChunkResult>, Error> {
